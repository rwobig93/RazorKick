...
public class RoleService : IRoleService
{
    ...
    async Task<IResult<PermissionsResponse>> GetAllPermissionsAsync(string roleId)
    {        try
        {
            ...
            var addedClaims = await _caimService.GetAllByRoleIdAsync(role.Id);
            if (addedClaims.Succeeded)
            {
                foreach (var claim in selectedClaims)
                {
                    ...
                    if (addedClaim != null)
                    {
                        ...
                        var saveResult = await _caimService.SaveAsync(claim);
    ...
    async Task<IResult<int>> GetCountAsync()
    {
        var count = await _roleManager.Roles.All();
        return Result<int>.SuccessAsync(count);
    }
    ...
    {
        var roles = await _roleManager.Roles.SingleOrDefaultAsync(x => x.Name == roleName);
        var rolesResponse = _mapper.Map<RoleResponse>(roles);
        return await Result<RoleResponse>.SuccessAsync(rolesResponse);
    }
    ...
    public async Task<Result<string>> SaveAsync(RoleRequest request)
    {
        if (string.IsNullOrEmpty(request.Id))
        {
            ...
            var response = await _roleManager.CreateAsync(new WobigTechRole(request.Name, request.Description));
