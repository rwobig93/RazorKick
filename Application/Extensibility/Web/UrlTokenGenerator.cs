...
namespace Application.Extensibility.Web;

public class UrlTokenGenerator : IDisposable
{
    private RandomNumberGenerator _numberGenerator;
    private readonly int _numberOfBytes;

    public UrlTokenGenerator(int numberOfBytes)
    {
    ...
    {
        using var factory = new UrlTokenGenerator(numberOfBytes);
        while (tokenCount > 0)
        {
            tokenCount--;
